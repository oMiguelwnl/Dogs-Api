...
function api_password_lost($request) {
  ...
  // Verifica se o email ou login foi fornecido
  if (empty($login)) {
  ...
  if (empty($user)) {
    // Se não encontrado, obtém o usuário pelo login
    $user = get_user_by('login', $login);
  }
  if (empty($user)) {
    // Se ainda não encontrado, retorna um erro
...
// Registra a rota da API para solicitar redefinição de senha
...
// Registra a rota da API para redefinir a senha
